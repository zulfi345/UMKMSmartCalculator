...
class MainActivity : Activity() {
    ...
    private fun showDashboard() {
        ...
        for (tx in transactions) {
            if (tx.date==today() && tx.type=="Penjualan") {
                todaySales += tx.amount
            }
            if (tx.date==today() && tx.type=="Pengeluaran") {
    ...
    private fun checkout() { if(cart.isEmpty()) { toast("Keranjang masih kosong"); return }; var total=0
        for ((id,q) in cart) {
            val product=products.find { it.id==id }
            if (product!=null) total+=product.sellPrice*q
        }; val methods=arrayOf("Tunai","Transfer","QRIS"); AlertDialog.Builder(this@MainActivity).setTitle("Pembayaran ${rupiah(total)}").setItems(methods){_,which-> confirmCheckout(methods[which],total)}.setNegativeButton("Batal",null).show() }
    ...
    private fun showSettings() { val root=screenRoot();root.addView(topBar("Pengaturan"){back()});val box=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(dp(16),dp(16),dp(16),dp(16))}; val items=listOf(Triple("Data Usaha","Nama usaha dan informasi toko",{navigate("business")}),Triple("Backup & Restore","Simpan dan pulihkan seluruh data",{navigate("backup")}),Triple("Notifikasi","Stok rendah dan pengingat",{navigate("notifications")}));items.forEach{box.addView(settingCard(it.first,it.second,it.third),margins(-1,-2,0,0,0,dp(8)))};box.addView(settingCard("Tema",if(darkMode)"Dark Mode aktif" else "Light Mode aktif"){darkMode=!darkMode;prefs.edit().putBoolean("dark",darkMode).apply();showSettings()},margins(-1,-2,0,0,0,dp(8)));box.addView(settingCard("Tentang Aplikasi","VECTA HUB UMKM • VECTA PROJECT"){AlertDialog.Builder(this@MainActivity).setTitle("VECTA HUB UMKM").setMessage("Aplikasi pengelolaan usaha UMKM. Versi 2.1").setPositiveButton("OK",null).show()});root.addView(box);root.addView(Space(this),LinearLayout.LayoutParams(-1,0,1f));root.addView(bottomNav("Lainnya"));setContentView(root) }
